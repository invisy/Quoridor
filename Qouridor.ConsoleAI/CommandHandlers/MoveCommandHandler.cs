...
namespace Qouridor.ConsoleAI.CommandHandlers
{
    public class MoveCommandHandler : ICommandHandler
    {
        ...
        public MoveCommandHandler(Action<MoveHorizontalNotation, int> action)
        {
            _action = action;
        }
        ...
        public ICommand IdentifyCommand(string command)
        {
            if (command.Length > 0)
            {
                ...
                if (splittedCommand.Length == 2 && splittedCommand[0] == "move" && splittedCommand[1].Length == 2)
                {
                    ...
                    if (xParseStatus && yParseStatus && y >= 1 && y <= 9)
                        return new MoveCommand(x, y, _action);
                }
            }
